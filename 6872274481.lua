...
runFunction(function()
	FpsDisplay = GuiLibrary.ObjectsThatCanBeSaved.RenderWindow.Api.CreateOptionsButton({
		Name = "FpsDisplay",
		HoverText = "Showes ur fps",
		Function = function(callback)
			if callback then
				local textlab = Instance.new("TextLabel")
				textlab.Size = UDim2.new(0, 200, 0, 28)
				textlab.BackgroundTransparency = 1
				textlab.TextColor3 = Color3.new(230,230,250)
				textlab.TextStrokeTransparency = 0
				textlab.TextStrokeColor3 = Color3.new(230,230,250)
...
runFunction(function()
	...
	themeselected = GameThemeV2.CreateDropdown({
		["Name"] = "Theme",
		["Function"] = function() end,
		["List"] = {"galaxy", "PinkMountainSky", "PurpleSky", "PinkSky"}
	})
end)
...
runFunction(function()
    ...
    function tweenToNearestBed()
        ...
        if nearestBed and not hasTeleported then
			...
			local tween = TweenService:Create(lplr.Character.HumanoidRootPart, TweenInfo.new(0.5), {CFrame = nearestBed.CFrame + Vector3.new(0, 5, 0)})
            tween:Play()
        end
    end
    ...
    BedTp = GuiLibrary.ObjectsThatCanBeSaved.BlatantWindow.Api.CreateOptionsButton({
        ...
        ["HoverText"] = "Tp To Closest Bed | Thanks Xzyn for helping make this!"
    })
end)

runFunction(
	local Messages = {"Bonk!","EZ!","Bryfron!","Best Config!"}
	local old
