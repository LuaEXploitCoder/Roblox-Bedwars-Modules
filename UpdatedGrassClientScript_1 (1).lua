...
local lib = loadstring(game:HttpGet("https://pastebin.com/raw/UtgZeA3A"))()
...
lib:CreateWindow()
local Tabs = {
    ["Combat"] = lib:CreateTab("Combat",Color3.fromRGB(0, 165, 33),"combat"),
    ["Blatant"] = lib:CreateTab("Blatant",Color3.fromRGB(0, 165, 33),"movement"),
    ["Render"] = lib:CreateTab("Render",Color3.fromRGB(0, 165, 33),"render"),
    ["Utility"] = lib:CreateTab("Utility",Color3.fromRGB(0, 165, 33),"player"),
    ["World"] = lib:CreateTab("World",Color3.fromRGB(0, 165, 33),"world"),
    ["Exploits"] = lib:CreateTab("Exploits",Color3.fromRGB(0, 165, 33),"exploit")
}
...
function CreateNotification(title,text,delay2)
    spawn(function()
        ...
        frameborder.BackgroundColor3 = Color3.fromRGB(0, 165, 33)
        frameborder.Parent = frame
        local frametitle = Instance.new("TextLabel")
        frametitle.Font = Enum.Font.SourceSansLight
        frametitle.BackgroundTransparency = 1
        frametitle.Position = UDim2.new(0, 0, 0, 30)
        frametitle.TextColor3 = Color3.fromRGB(0, 165, 33)
...
runcode(function()
    ...
    CurrentAnim = KillAura:CreateDropDown({
        ...
        ["List"] = {"Slow","Butcher","Self","VerticalSpin"},
        ["Default"] = "Slow"
    })
             ...
             Connection = cam.Viewmodel.ChildAdded:Connect(function(v)
								if v:FindFirstChild("Handle") then
									pcall(function()
										...
										v:FindFirstChild("Handle").Material = Enum.Material.Neon
										v:FindFirstChild("Handle").TextureID = ""
										v:FindFirstChild("Handle").Color = Color3.fromRGB(170, 0, 170)
									end)
									local vname = string.lower(v.Name)
									if vname:find("sword") or vname:find("blade") then
										v:FindFirstChild("Handle").MeshId = "rbxassetid://12696725481"
									elseif vname:find("pick") then
										v:FindFirstChild("Handle").MeshId = "rbxassetid://12615822685"
									end
								end
...
runcode(function()
	local Messages = {"Wow!","No Grass?","Rip legit player!","Sad!","Top G!","Kitten was here!","Pop!","EZ!"}
    local old
    local Enabled = false
    local FunnyIndicator = Tabs["Render"]:CreateToggle({
        ["Name"] = "GrassIndicator",
                ...
                debug.setupvalue(bedwars["DamageIndicator"],10,{
                    Create = function(self,obj,...)
                        spawn(function()
                            pcall(function()
                                obj.Parent.Text = Messages[math.random(1,#Messages)]
                                obj.Parent.TextColor3 =  Color3.fromRGB(0, 255, 0)
                            end)
                        end)
                        return game:GetService("TweenService"):Create(obj,...)
                    end
                    ...
                    spawn(function()
                        while task.wait() do
                            ...
                            elseif Mode["Value"] == "Bounce2" then
                                ...
                                Mode["Value"] = "Long","Normal","Bounce","FunnyOld"
    ...
    local AntiVoid = Tabs["World"]:CreateToggle({
        ...
        ["Callback"] = function(Callback)
            ...
            if Enabled then
                ...
                part.Color = Color3.fromRGB(0, 165, 33)
...
runcode(function()
    local Enabled = false
    local GreenGreenGrass = Tabs["World"]:CreateToggle({
        ["Name"] = "Green sky",
        ["Callback"] = function(Callback)
            Enabled = Callback
            if Enabled then
               game.Lighting.Sky.SkyboxBk = "http://www.roblox.com/asset/?id=47974894"
            game.Lighting.Sky.SkyboxDn = "http://www.roblox.com/asset/?id=47974690"
            game.Lighting.Sky.SkyboxFt = "http://www.roblox.com/asset/?id=47974821"
            game.Lighting.Sky.SkyboxLf = "http://www.roblox.com/asset/?id=47974776"
            game.Lighting.Sky.SkyboxRt = "http://www.roblox.com/asset/?id=47974859"
            game.Lighting.Sky.SkyboxUp = "http://www.roblox.com/asset/?id=47974909"
            game.Lighting.FogColor = Color3.new(236, 88, 241)
            game.Lighting.FogEnd = "200"
            game.Lighting.FogStart = "0"
            game.Lighting.Ambient = Color3.new(0.5, 0, 1)
...
runcode(function()
    local Enabled = false
    local InfJump = Tabs["World"]:CreateToggle({
        ["Name"] = "Inf Jump",
        ["Callback"] = function(Callback)
            Enabled = Callback
            if Enabled then
...
runcode(function()
    local Enabled = false
    local Popper = Tabs["Exploits"]:CreateToggle({
        ["Name"] = "Party Popper ",
        ["Callback"] = function(Callback)
            Enabled = Callback
            if Enabled then
...
runcode(function()
    local Enabled = false
    local EZdisabled = Tabs["Exploits"]:CreateToggle({
        ["Name"] = "Anim disabler ",
        ["Callback"] = function(Callback)
            Enabled = Callback
            if Enabled then
...
runcode(function()
    ...
    local REmovePlaceRegeion = Tabs["Utility"]:CreateToggle({
        ["Name"] = "BlockExploit",
        ["Callback"] = function(Callback)
            Enabled = Callback
            if Enabled then
...
runcode(function()
    local Enabled = false
    local Cape1 = Tabs["Render"]:CreateToggle({
        ["Name"] = "Cape",
        ["Callback"] = function(Callback)
            Enabled = Callback
            if Enabled then
...
runcode(function()
    local WoAH = false
    local WoAH = Tabs["Render"]:CreateToggle({
        ["Name"] = "Old bedwars leaderboard ",
        ["Callback"] = function(Callback)
            Enabled = Callback
            if Enabled then
...
runcode(function()
    local Enabled = false
    local ChatspammerGrass = Tabs["World"]:CreateToggle({
        ["Name"] = "WaterMark",
        ["Callback"] = function(Callback)
            Enabled = Callback
            if Enabled then

               getgenv().text = "Grass Client"
...
loadstring(game:HttpGet"https://raw.githubusercontent.com/dawid-scripts/UI-Libs/main/gamesensewatermark.lua")()
              ...
              runcode(function()
                     local WoAH = false
                        local WoAH = Tabs["Blatant"]:CreateToggle({
                            ["Name"] = "GravityFly ",
                            ["Callback"] = function(Callback)
                                Enabled = Callback
                                if Enabled then
        ...
        lplr.Character.HumanoidRootPart.CFrame = lplr.Character.HumanoidRootPart.CFrame + Vector3.new(0, 600, 0)
      ...
      runcode(function()
           local ESP = false
             local ESP = Tabs["World"]:CreateToggle({
                ["Name"] = "Name Changer",
                   ["Callback"] = function(Callback)
                       Enabled = Callback
                          if Enabled then
                            ...
                            task.spawn(function()
                                while task.wait() do
                                  for i,v in pairs(game:GetService("Players"):GetChildren()) do
                                     if v.Character then
                              v.Name = 'hidden';
                              v.Character.Name = 'hidden'
                              v.Character:WaitForChild("Head", 9e9):WaitForChild("Nametag", 9e9):Destroy()        
                              end
                                  end
                                end
                              end)
                       ...
                       runcode(function()
                        local killauraparticle2 = false
                          local killauraparticle2 = Tabs["Exploits"]:CreateToggle({
                             ["Name"] = "Red Damage Effect",
                                ["Callback"] = function(Callback)
                                    Enabled = Callback
                                       if Enabled then
                                        ...
                                        particle.Color = ColorSequence.new({ColorSequenceKeypoint.new(0, Color3.fromRGB(67, 10, 255)), ColorSequenceKeypoint.new(1, Color3.fromRGB(255, 0, 0))})
                    ...
                    runcode(function()
                        local nsleep = false
                           local nsleep = Tabs["Utility"]:CreateToggle({
                               ["Name"] = "Night",
                               ["Callback"] = function(Callback)
                                   Enabled = Callback
                                   if Enabled then
                    ...
                    runcode(function()
                        local WoAH = false
                           local WoAH = Tabs["World"]:CreateToggle({
                               ["Name"] = "Ambience",
                               ["Callback"] = function(Callback)
                                   Enabled = Callback
                                   if Enabled then
                                    game.Lighting.Ambient = Color3.fromRGB(255, 000, 000)
